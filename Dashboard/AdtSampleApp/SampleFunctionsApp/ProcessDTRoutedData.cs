        ...
        public static async Task Run([EventGridTrigger] EventGridEvent eventGridEvent, ILogger log)
        {
            ...
            if (client != null)
            {
                if (eventGridEvent != null && eventGridEvent.Data != null)
                {
                    ...
                    string parentId = await AdtUtilities.FindParentAsync(client, twinId, "hasSmartDetection", log);
                    log.LogInformation("Twins ParentId -- > " + parentId);

                    if (parentId != null)
                    {
                        //log.LogInformation("Operations list -- > "+ message["data"]["patch"].ToString());
                        // Read properties which values have been changed in each operation
                        foreach (var operation in message["data"]["patch"])
                        {
                            string opValue = (string)operation["op"];
                            //log.LogInformation("Operations value -- > " + opValue);

                            if (opValue.Equals("add"))
                            {
                                string propertyPath = ((string)operation["path"]);
                                log.LogInformation("Property Path -- > " + propertyPath);
                                ...
                                if (propertyPath.Equals("/locationinfo/lat"))
                                {
                                    await AdtUtilities.UpdateTwinPropertyAsync(client, parentId, propertyPath, operation["value"].Value<float>(), log);
                                }
                                if (propertyPath.Equals("/locationinfo/lon"))
                                {
                                    await AdtUtilities.UpdateTwinPropertyAsync(client, parentId, propertyPath, operation["value"].Value<float>(), log);
                                }
                                if (propertyPath.Equals("/locationinfo/timezone"))
                                {
